...
% to study opposition of phase between two conditions. 
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                                   Usage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%All Settings are below. This script extracts the filtered phase and
%amplitude of the frequencies that are within a significant cluster of PAC
%(see PAC_avg_img.m) and then determines an average distribution of mean
%amplitude per pahse bin for the whole cluster (averaged over trials and
%pairs of frequencies). It does so for 2 conditions, and then compares the
%angle of preference of the cluster of 2 distributions with the MOVI (see
%MOVI_help) It also shows a Polar plot of the average amplitude per pahse
%bin and a distribution plot with std of patients taken into account, and a
%bar plot that shows MOVI values compared to surrogate trials. 
%Surrogate trials are calculated by shuffling the labels of trial numbers
%of the aplitude data before binning. 
%It also shows the distrubtion and angle of preference for each participant
...
event1              = [50];    %eventvalues of the event: 10 11 12 13 = images 1 2 3 4
...
day1                = 2;                %day of when the eventvalue takes place, it can be 1 or 2 for both days
day2                = 2;
...
filename= sprintf('MOVI_como_day%d_%d_%s_%s_%s_%s_%s_%s%s.mat', day1, day2,...
    mat2str(event1), acc1, mat2str(event2), acc2, method, config.norm, roi);
...
AB2 = ampbin2;
toc
filename = sprintf('MOVI_BC_%s_%s_day%d_%s_%s_day%d_Clust_%s%s%s.mat'...
    , ev1, acc1, day1, ev2, acc2, day2, clusttype, roi, method);
...
for subj = 1:numel(subjc)
    ampz1_z(subj, :) = smoothbin(ampz1_z(subj, :), 5);
    ampz2_z(subj, :) = smoothbin(ampz2_z(subj, :), 5);
end
